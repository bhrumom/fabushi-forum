...
function buildSkipNextStepLines(target) {
  return [
    ...
    "```",
    "",
    "That rollout command can scaffold `.env.deploy`, validate the deploy posture, bring the compose stack up, wait for health, and smoke-check the runtime.",
    "That handoff command re-validates `.env.deploy`, smoke-checks the live runtime, and syncs `FORUM_LIVE_TARGET` back to `bhrumom/fabushi`.",
