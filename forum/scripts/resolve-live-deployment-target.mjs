...
function buildSkipNextStepLines(target) {
  return [
    ...
    "```",
    "",
    "If the target host is preview and `.env.deploy` already sets `FORUM_DEPLOY_CHECK_URL`, the shorter deploy-env-driven handoff path is:",
    "",
    "```bash",
    target.recommendedPreviewHandoffCommand,
    "```",
    "",
...
async function main() {
  ...
  if (!forumUrl) {
    const skippedTarget = {
      ...
      recommendedPreviewHandoffCommand: [
        "cd forum",
        "pnpm handoff:live-target -- \\",
        "  --deploy-env-path .env.deploy \\",
        "  --apply-github-live-target true",
